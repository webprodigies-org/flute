...
type DashboardData = {
  ...
  customers: number;
  points: number[];
  activity: { name: string; detail: string; amount: string }[];
};
...
export function DashboardProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/dashboard", { signal: controller.signal })
      .then((r) => {
        if (!r.ok) throw new Error("Dashboard data is unavailable.");
        return r.json();
      })
      .then(setData)
      .catch((e) => {
...
export function useDashboard() {
  ...
  if (!state) throw new Error("DashboardProvider is required.");
  return state;
}
