      ...
      tracks: [
        {
          target: { kind: "camera" },
          property: "x",
          keyframes: keyframes(0, 115, -55),
        },
        {
          target: { kind: "camera" },
          property: "y",
          keyframes: keyframes(0, -35, 25),
        },
        {
          target: { kind: "camera" },
          property: "rotateY",
          keyframes: keyframes(-20, 8, -12),
        },
        {
          target: { kind: "surface", id: "folder-front" },
          property: "x",
          keyframes: keyframes(-35, 100, 30),
        },
        {
          target: { kind: "surface", id: "folder-front" },
          property: "z",
          keyframes: keyframes(260, -70, 180),
        },
        {
          target: { kind: "surface", id: "folder-rear" },
          property: "y",
          keyframes: keyframes(0, -65, 15),
        },
        {
          target: { kind: "surface", id: "customers" },
          property: "z",
          keyframes: keyframes(95, 165, 60),
        },
      ],
    };
  ...
  if (error)
    return (
      <div className="data-state" role="alert">
        <h2>Let’s reconnect your dashboard.</h2>
        <p>{error}</p>
...
function Folder({
  ...
  return (
    <div className={"folder " + (front ? "folder-front" : "")}>
      <svg viewBox="0 0 150 116" fill="none" aria-hidden="true">
        <path
          d="M8 19a9 9 0 0 1 9-9h38l15 14h63a9 9 0 0 1 9 9v66a9 9 0 0 1-9 9H17a9 9 0 0 1-9-9Z"
          fill={front ? "#9183bb" : "#72758d"}
        />
        <path
          d="M8 40a9 9 0 0 1 9-9h116a9 9 0 0 1 9 9v59a9 9 0 0 1-9 9H17a9 9 0 0 1-9-9Z"
          fill={front ? "#b6a9db" : "#a5a8bb"}
        />
...
function StorageStudy() {
  ...
  const [fault, setFault] = useState(
    fixture === "duplicate"
      ? "duplicate"
      : fixture === "invalid" || fixture === "missing"
        ? "invalid"
        : "",
  );
  ...
  useEffect(() => {
    if (timeMs >= durationMs) setPlaying(false);
  }, [timeMs]);
  const reset = () => {
    setPlaying(false);
    setTimeMs(0);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-layout">
        <section className="canvas-area">
          <div className="canvas-title">
            <div>
              <p className="eyebrow">STUDY 002 — INDEPENDENT FOCUS</p>
              <h1>
                A different
                <br />
                <em>point of view.</em>
              ...
              <p className="intro">
                Move the scene. Let focus have a life of its own.
              </p>
            </div>
          ...
          <div className="stage-viewport">
            <div className="stage-frame">
              ...
              <HostDataState>
                <SceneErrorBoundary resetKey={revision}>
                  <Scene
                    className="scene"
                    camera={camera}
                    focus={{
                  ...
                  >
                    <Surface id="composition" className="composition">
                      <Surface
                        id="storage"
                        className="storage-surface"
                        transform={{ z: -85 }}
                        content={
                          <div className="storage-panel">
                            <div className="storage-nav">
                              <span className="storage-logo">n /</span>
                              <span>Workspace</span>
                              <b>▣ Storage</b>
                              <span>Overview</span>
                              <span>Activity</span>
                      ...
                      >
                        <Surface
                          id="folder-rear"
                          className="rear-folder"
                          content={
                            <Folder title="Projects" detail="12 collections" />
                          }
                        />
                        <Surface
                          id="folder-middle"
                          className="middle-folder"
                          transform={{ z: 45 }}
                          content={
                            <Folder
                              title="Resources"
                      ...
                      <Surface
                        id="folder-front"
                        className="foreground-folder"
                        transform={{ x: -35, z: 260, rotateZ: -7 }}
                        content={
                          <Folder
                            title="Ideas"
                            detail="Room to explore"
                            front
                          />
                        }
                      />
                      <Surface id="revenue" className="revenue-surface">
                        <SceneRevenueCard />
                      </Surface>
                      <Surface
                        id={fault === "duplicate" ? "revenue" : "customers"}
                        className="customers-surface"
                        transform={{ z: 95 }}
                      >
                        <CustomersCard />
                      </Surface>
              ...
              <div className="stage-bottom">
                ...
                <span>PERSPECTIVE / DEPTH / FOCUS</span>
              </div>
            </div>
          </div>
          <div className="transport">
            <button
              ...
              aria-label={
                playing ? "Pause" : timeMs >= durationMs ? "Replay" : "Play"
              }
              onClick={() => {
            ...
            <label className="timeline">
              <span className="sr-only">Scene time</span>
              <input
                type="range"
            ...
            <output className="timecode" data-testid="scene-time">
          ...
          <div className="canvas-footer">
            <span>
              {recipe === "fixed"
                ? "Focus stays still. Camera and layers move through it."
                : recipe === "travel"
                  ? "Camera stays still. Focus travels and tightens."
                  : "Manual composition. Every control is independent."}
            </span>
            <span>
              {reduced
          ...
          <details className="proof-details">
            <summary>About this live scene</summary>
            <p>
              The Revenue, Customers and Activity cards use the original host
              provider and API. Try their controls, then move the scene: their
            ...
            <div className="proof-actions">
              ...
              <button onClick={() => setFault("")}>Restore valid scene</button>
              <a href="?fixture=baseline">Open host baseline ↗</a>
            </div>
          </details>
        </section>
        <aside className="inspector" aria-label="Scene controls">
          ...
          <p className="inspector-intro">
            One scene. Two independent movements.
          </p>
          <section className="control-section">
            <h3>
              Choreography <span>01</span>
            </h3>
            <div className="recipe-options">
              {(
                [
                  {
                    id: "fixed",
                    title: "Move through focus",
                    detail: "Fixed focus · camera & layers move",
                  },
                  {
                    id: "travel",
                    title: "Let focus wander",
                    detail: "Still camera · focus travels & tightens",
                  },
                ] as const
              ).map((item) => (
                <button
            ...
            <p className="control-hint">
              Adjusting a slider pauses the recipe and opens manual composition.
            </p>
          </section>
          <section className="control-section">
            <h3>
              Focal point <span>02</span>
            </h3>
            <div className="slider-grid">
              {(
                ...
                <Slider
                  ...
                  min={item.min}
                  max={item.max}
                  value={focus[item.key]}
                  onChange={(value) => {
                    manual();
                    setFocus((current) => ({ ...current, [item.key]: value }));
                  }}
                />
              ))}
            </div>
          ...
          <section className="control-section">
            ...
            <div className="slider-grid">
              {(
                [
                  { key: "x", label: "Camera horizontal", min: -250, max: 250 },
                  { key: "y", label: "Camera vertical", min: -180, max: 180 },
                  { key: "z", label: "Camera depth", min: -250, max: 250 },
                  {
                    key: "rotateY",
                  ...
                  { key: "rotateX", label: "Vertical tilt", min: -30, max: 30 },
                ] as const
              ).map((item) => (
                <Slider
                  key={item.key}
                  ...
                  min={item.min}
                  max={item.max}
                  unit={item.key.startsWith("rotate") ? "°" : "px"}
                  value={camera[item.key]}
                  onChange={(value) => {
                    manual();
                    setCamera((current) => ({ ...current, [item.key]: value }));
                  }}
                />
              ))}
            </div>
          ...
          {issues.length > 0 && (
            <div className="diagnostics" role="status">
              <strong>Scene needs a correction.</strong>
      ...
      <footer className="bottom-bar">
        ...
        <span>Real components. A new dimension.</span>
        <span>FLUTE / 002</span>
      </footer>
    </div>
  );
...
export default function App() {
  const baseline =
    new URLSearchParams(window.location.search).get("fixture") === "baseline";
  return (
    <DashboardProvider>
      {baseline ? (
        <HostDataState>
