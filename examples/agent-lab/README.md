# Current preview: camera studies, revision 3

The original independent-worker shots below are historical evidence, not the current gallery. After the user rejected their framing, the coordinator replaced them with `?scene=surface-travel`, `?scene=plating` and `?scene=floating`. Old query names resolve to these new shots. All use fixed oblique angles, camera rails derived with the installed matrix helper, camera-based depth of field and black void. Plating settles depth-separated original sections into their unchanged dashboard slots. Floating keeps a lifted chart and the real page behind it. The dashboard exposes a renderSection callback; normal rendering is unchanged, no duplicate UI is introduced. Layout ancestors carry preserve-3d so section depth is rendered rather than flattened.

Revision 3 migrates all scenes to the version 3 lens contract: camera focus distance, f-stop and gentle maximum blur. Sidebar and header are registered visual leaves alongside metrics, chart and table, so their text cannot bypass focus when other sections lift. Equal camera-depth regions can correctly remain sharp; aperture-shaped bokeh remains a live-DOM approximation.
