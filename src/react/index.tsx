...
export function Surface({ id, transform, children, content, className, style }: SurfaceProps) {
  ...
  useLayout(() => { registry.upsert({ token, id, parent, element: element.current!, transform }); });
...
export class SceneErrorBoundary extends Component<SceneErrorBoundaryProps, ErrorState> {
  state: ErrorState = { error: null };
  static getDerivedStateFromError(error: unknown): ErrorState { return { error: error instanceof Error ? error : new Error(String(error)) }; }
  componentDidUpdate(previous: SceneErrorBoundaryProps) {
    if (!Object.is(previous.resetKey, this.props.resetKey) && this.state.error) this.setState({ error: null });
  }
  render() {
    return this.state.error ? <div role="alert"><strong>Unable to render the Flute scene.</strong><p>{this.state.error.message}</p><p>Correct the component or scene configuration, then retry.</p><button type="button" onClick={() => this.setState({ error: null })}>Retry scene</button></div> : this.props.children;
  }
}
