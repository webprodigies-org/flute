...
export function getAuthoringGuide(): AuthoringGuide {
  ...
  return AuthoringGuideSchema.parse({
    ...
    workflow:[
      ...
      "Compose real components as stable Surface children of ScenePreview from @flute/scene/preview, using canonical motion helpers and matching scene node metadata. Keep the normal app available. Use an authored scene route without flute-preview=1: that query activates the separate ProjectPreview bootstrap around the whole app. Do not nest the two previews. Saved recipe discovery is not implemented.",
