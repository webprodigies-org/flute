...
const concepts: z.infer<typeof ConceptSchema>[] = [
 ...
 choices:"For the default dramatic shot, establish a visibly sharp subject and visibly softer nearer/farther regions in the same frame. Tune xyz depth to the subject plane, then tighten radius until adjacent content visibly crosses the falloff. Do not accept a configured blur value as proof that blur is visible. Keep the field screen-locked while camera travel brings content through it, or deliberately animate focus independently. Fully sharp shots are an explicit creative variation, not the default demonstration of this product.",
 ...
 verify:"Approve a still frame before animation: visible near/far size difference, dramatic crop, black void and local sharp-to-soft transition. At several times verify the page pose is unchanged and the camera follows the intended edge without drifting away from the subject."},
 ...
 verify:"Inspect both the dramatic starting composition and final registration against the original page. Check visible relative depth during convergence, progressive focus crossing and no overlap errors in the assembled result."},
 ...
 mechanism:"Use camera distance and translation to frame the existing detail on an oblique plane. Keep its host subtree intact. Place the focus field at that detail's camera-space depth with a tight clear radius and gradual falloff; travel a short distance along the plane.",
 ...
 mechanism:"Place the existing foreground subtree on a nearer Surface and retain a distinct related background Surface. Translate the camera so their projected positions change differently. Gentle foreground motion may supplement that move. Use the canonical focus field to distinguish depth without erasing all background structure.",
