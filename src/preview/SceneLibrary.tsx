...
export function SceneLibrary({sources=EMPTY_SOURCES,bindings=EMPTY_BINDINGS,hot,backHref}:SceneLibraryProps){
 ...
 useEffect(()=>{if(selected&&Component)return;const element=scroller.current;if(!element)return;const measure=()=>setWidth(element.clientWidth);const observer=new ResizeObserver(measure);observer.observe(element);measure();element.scrollTop=savedScroll.current;return()=>observer.disconnect()},[selected?.id,Component]);
