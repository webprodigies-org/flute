...
Automatic setup supports the documented npm/Vite/React 19.2 configuration boundary. Public npm distribution is now authorized: viewers must be able to install the library through a normal npm command and use the same agent guide and studio. Release preparation is in progress; publisher ownership, registry publication and fresh public installation must be verified before claiming viewer readiness. The guide teaches cinematic relationships and shared APIs, leaving composition and artistic overrides to the creator. The executable example proves wiring rather than imposing a common design. Future plans remain deferred and are not part of this MVP.
