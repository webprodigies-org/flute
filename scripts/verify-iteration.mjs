...
export async function verifyIteration({page,host,origin,server,processes,waitFor,root,originalApp}) {
 ...
 await writeFile(file('scene.ts'),sceneSource(1800));
 await writeFile(file('App.tsx'),shell);
 ...
 await writeFile(file('scene.ts'),sceneSource(1600));
 ...
 await writeFile(file('scene.ts'),sceneSource(-1));
 await expect(page.getByRole('alert')).toContainText('last valid');
 await expect(page.getByRole('button',{name:'Play',exact:true})).toBeDisabled();
 await writeFile(file('scene.ts'),sceneSource(1600));
 await expect(page.getByRole('alert')).toHaveCount(0);
 await expect(seek).toHaveValue('4000');
 await writeFile(file('scene.ts'),'export const definition = ;');
 await expect(page.locator('vite-error-overlay')).toHaveCount(1);
 await writeFile(file('scene.ts'),sceneSource(1600));
 await expect(page.locator('vite-error-overlay')).toHaveCount(0);
 await expect(page.getByRole('alert')).toHaveCount(0);
 await expect(seek).toHaveValue('4000');
 await writeFile(file('Host.tsx'),originalApp.replace('  const revenue =', '  throw new Error("Fixture render failure");\n  const revenue ='));
 ...
 await writeFile(file('App.tsx'),originalApp);
