...
try {
 ...
 await seek(2600);
 assert.match(await page.locator('[data-flute-id="brand"]').getAttribute('style'),/translate3d\(0px, 0px, 0px\)/);
 assert.match(await page.locator('[data-flute-id="team"]').getAttribute('style'),/translate3d\(0px, 0px, 190px\)/);
 await seek(11000);
 ...
 await seek(5500);await page.screenshot({path:'test-results/dashboard-mid.png'});
 await seek(11000);await page.screenshot({path:'test-results/dashboard-end.png'});
 ...
 assert.equal(await page.getByTestId('scene-time').textContent(),'0.0 / 11s');
