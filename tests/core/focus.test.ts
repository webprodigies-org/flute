...
it("varies blur continuously within one tilted live plane and symmetrically across focus depth", () => {
  const node = evaluateScene({
    focus: { radius: 20, falloff: 180, maxBlur: 12 },
    nodes: [{ id: "a", transform: { rotateX: 25, rotateY: 35 } }],
  }).nodes[0];
  ...
  const front = evaluateScene({
    focus: { radius: 0 },
    nodes: [{ id: "a", transform: { z: 80 } }],
  }).nodes[0];
  const back = evaluateScene({
    focus: { radius: 0 },
    nodes: [{ id: "a", transform: { z: -80 } }],
  }).nodes[0];
...
it("keeps focus independent of surface identity and fixed under camera movement", () => {
  const scene = {
    focus: { x: 50, y: 20, z: 0, radius: 0, falloff: 100 },
    nodes: [{ id: "a", transform: { x: 50, y: 20 } }],
  };
...
it.each([
  ...
  { camera: { z: NaN } },
  { focus: { targetId: "old" } },
])("rejects invalid or obsolete spatial configuration", (input) => {
...
it("rejects degenerate accumulated scales before emitting unusable masks", () => {
  const result = evaluateScene({
    nodes: [{ id: "tiny", transform: { scale: 1e-100 } }],
  });
