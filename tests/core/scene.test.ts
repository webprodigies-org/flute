...
describe('versioned scene contract', () => {
  it('derives defaults and preserves a data-only serializable recipe', () => {
    ...
    if (result.success) {
      ...
      expect(validateScene(JSON.parse(JSON.stringify(result.data)))).toEqual(result);
    }
  });
  it.each([
    {version:2,nodes:[]}, {nodes:[{id:'a'},{id:'a'}]}, {nodes:[{id:'a',parentId:'b'}]},
    {nodes:[{id:'a',parentId:'b'},{id:'b',parentId:'a'}]},
    {nodes:[{id:'a',transform:{z:Infinity}}]}, {nodes:[{id:'a',transform:{scale:0}}]},
    {nodes:[],focus:{targetId:'absent'}}, {nodes:[],token:'secret'}, {nodes:[{id:'a',component:()=>null}]},
...
describe('spatial evaluation', () => {
  it('changes focal sharpness across three depths and clamps blur', () => {
    const nodes = [-200,0,200].map((z,i)=>({id:'n'+i,transform:{z}}));
    const center=evaluateScene({nodes,focus:{targetId:'n1',range:0,falloff:50,maxBlur:3}});
    expect(center.nodes.map(n=>n.blur)).toEqual([3,0,3]);
    const near=evaluateScene({nodes,focus:{targetId:'n2',range:0,falloff:50,maxBlur:3}});
    expect(near.nodes.map(n=>n.blur)).toEqual([3,3,0]);
  });
  it('composes parent rotation, layout offset and child depth', () => {
    const result=evaluateScene({nodes:[{id:'p',transform:{rotateY:90,z:100}},{id:'c',parentId:'p',transform:{z:20}}]}, {c:{width:10,height:10,offsetX:30}});
  ...
  it('includes the camera rotation when resolving a focal target', () => {
    const result=evaluateScene({camera:{rotateY:90},focus:{targetId:'a'},nodes:[{id:'a',transform:{x:100}}]});
  ...
  it('handles missing measurements and diagnoses invalid/zero area and camera collision', () => {
    ...
    const result=evaluateScene({camera:{perspective:100},nodes:[{id:'a',transform:{z:100}}]}, {a:{width:0,height:10}});
  ...
  it('keeps CSS transform order aligned with world transform composition', () => {
    expect(transformToCss({x:2,rotateY:30})).toBe('translate3d(2px, 0px, 0px) rotateX(0deg) rotateY(30deg) rotateZ(0deg) scale(1)');
  });
});
