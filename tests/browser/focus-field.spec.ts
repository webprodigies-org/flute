...
test("live pixels are sharp at independent focus and progressively blurred across one plane", async ({
  page,
}) => {
...
test("focus stays on screen while surface and camera move; focus can then travel independently", async ({
  page,
}) => {
  await page.setViewportSize({ width: 800, height: 400 });
  await page.goto("/tests/fixtures/focus.html");
  ...
  expect(await contrast(570)).toBeLessThan(10);
  ...
  const near = await contrast(470),
    far = await contrast(260);
  expect(near).toBeGreaterThan(far + 40);
});
